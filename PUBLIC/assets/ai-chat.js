(function () {
  ...
  window.toggleAiChat = () => {
    ...
    if (!w.dataset.greeted) {
      const username = window.JMS?.store?.me?.username || '';
      const greetWord = getGreeting(); // e.g., "Namaste"
      const namePart = username ? ` ${username}` : '';
      const page = document.title.replace('JMS', '').replace('–', '').replace('-', '').trim() || 'Dashboard';
  ...
  const sendMsg = async () => {
    ...
    try {
      const username = window.JMS?.store?.me?.username || 'Friend';
      ...
      const res = await window.JMS.api.post('/ai/ask', { question: txt, username, context });
      removeMsg(thinkId);
