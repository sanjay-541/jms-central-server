...
let SAVED_IPS = JSON.parse(localStorage.getItem('JMS_SAVED_IPS') || '[]');
let CURRENT_FILTER = localStorage.getItem('JMS_SCAN_FILTER') || '';
...
async function initScanning(renderCallback) {
    JMS.auth.requireAuth();
    JMS.renderShell('packing');

    // 0. Fetch Lines (Configs)
    try {
        const res = await JMS.api.get('/assembly/lines');
        ALL_LINES = res.data || [];
        renderFilter();
...
async function loadActivePlans(renderCallback) {
    try {
        const res = await JMS.api.get('/assembly/active');
...
function connectToBridge(renderCallback) {
    ...
    try {
        ...
        ws.onopen = () => {
            ...
            USING_BRIDGE = true;
            BRIDGE_WS = ws;
            JMS.toast('Connected to Local Scanner Bridge', 'success');
...
function handleBridgeMessage(msg, renderCallback) {
    if (msg.type === 'ports') {
        ...
        JMS.toast(`Bridge Error: ${errText}`, 'error');
    }
}
...
function renderFilter() {
    ...
    if (filter) {
        ...
        filter.onchange = (e) => {
            ...
            localStorage.setItem('JMS_SCAN_FILTER', CURRENT_FILTER);
            if (window.renderCards) window.renderCards();
        };
    }
}
    ...
    ALL_LINES.forEach(line => {
        ...
        if (type === 'IP') {
            if (USING_BRIDGE && BRIDGE_WS) {
                ...
                JMS.toast(`Auto-Connecting ${tid} to ${val}`, 'info');
            }
        }
        else if (type === 'BRIDGE') {
            if (USING_BRIDGE && BRIDGE_WS) {
...
async function connectScanner(tid, renderCallback) {
    ...
    if (val === '-1') return JMS.toast('Please select a port/IP first', 'warn');

    // CLEAR
    if (val === 'CLEAR_IPS') {
        if (confirm('Clear all saved IPs?')) {
            SAVED_IPS = [];
            localStorage.removeItem('JMS_SAVED_IPS');
            if (window.renderCards) window.renderCards();
            JMS.toast('Saved IPs cleared', 'success');
        }
        return;
    }
    ...
    if (val === 'NEW_IP') {
        ...
        if (!ip) return;
        if (!ip.includes(':')) return JMS.toast("Format must be IP:PORT", 'error');

        // Save
        if (!SAVED_IPS.includes(ip)) {
            SAVED_IPS.push(ip);
            localStorage.setItem('JMS_SAVED_IPS', JSON.stringify(SAVED_IPS));
            if (window.renderCards) window.renderCards();
        }
    ...
    // NEW SERIAL
    if (val === 'NEW_SERIAL') {
        if (!('serial' in navigator)) return JMS.toast('Web Serial not supported.', 'error');
        try {
...
async function connectBridgePort(tid, comPath) {
    if (!BRIDGE_WS) return JMS.toast('Bridge not connected!', 'error');
...
function connectTcpPort(tid, ipStr) {
    if (!BRIDGE_WS) return JMS.toast('Bridge not connected! Cannot use IP.', 'error');
...
async function processScan(tid, ean) {
    ...
    if (LAST_SCANS[tid] === ean) {
        ...
        JMS.toast('You are scanning Same product twice!', 'error');
    ...
    */
    ...
    try {
        const res = await JMS.api.post('/assembly/scan', { plan_id: planId, ean });
        if (res.ok && res.match) {
